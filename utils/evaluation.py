...
def evaluate(
    ...
    for i in trange(num_eval_episodes + num_video_episodes):
        ...
        if complex_task_name is not None:
            ...
            options["task_info"] = {
                "init_ij": reward_cfg["initial_state"],
                "goal_ij": reward_cfg["argmax"],
            }
            current_return = 0
        ...
        while not done:
            if inferred_latent is not None:
                action = actor_fn(
                    observation, inferred_latent, temperature=eval_temperature
                )
            else:
            ...
            if complex_task_name is not None:
                next_observation_dict = {
                    "xy_pos": next_observation[
                        None, qpos_xy_start_idx : qpos_xy_start_idx + 2
                    ],
                    "xy_vel": next_observation[
                        None, qvel_xy_start_idx : qvel_xy_start_idx + 2
                    ],
                }
                reward = complex_rewards_maze(
                    env, next_observation_dict, task_id, complex_task_name
                )[
                    0
                ]  # relabelling reward using true reward function
                    ...
                    episode_info.update(
                        {
                            ...
                            "to_goal_discounted_return": float(
                                current_discounted_return
                            ),
                            "in_goal_discounted_return": reward
                            * agent.config["discount"] ** (step - 1)
                            / (1 - agent.config["discount"]),
            ...
            if complex_task_name is not None:
                current_discounted_return += reward * agent.config["discount"] ** (
                    step - 1
                )
