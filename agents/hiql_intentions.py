...
from utils.networks import MLP, GCActor, GCBilinearValue, LengthNormalize
...
class HIQLwithIntentionsAgent(flax.struct.PyTreeNode):
    ...
    def value_loss(self, batch, grad_params):
        ...
        intention_rewards = ((observations == intention_goals).all(axis=1)).astype(
            float
        )
        ...
        # TD-error
        (next_v1_gz, next_v2_gz) = self.network.select("target_value")(
            next_observations, value_goals, intention_goals
        )
        ...
        (v1_gz, v2_gz) = self.network.select("value")(
            observations, value_goals, intention_goals, params=grad_params
        )

        # Direction-error
        (next_v1_zz, next_v2_zz) = self.network.select("value")(
            next_observations, intention_goals, intention_goals, params=grad_params
        )
        next_v_zz = jnp.minimum(next_v1_zz, next_v2_zz)
        q_zz = intention_rewards + discount * next_v_zz

        (v1_zz, v2_zz) = self.network.select("value")(
            observations, intention_goals, intention_goals, params=grad_params
        )
        v_zz = (v1_zz + v2_zz) / 2
    ...
    def actor_loss(self, batch, grad_params):
        ...
        v1, v2 = self.network.select("value")(
            batch["observations"], batch["actor_goals"], batch["actor_goals"]
        )
        nv1, nv2 = self.network.select("value")(
            batch["next_observations"], batch["actor_goals"], batch["actor_goals"]
        )
        v = (v1 + v2) / 2
        nv = (nv1 + nv2) / 2
    ...
    def create(cls, seed, ex_batch, config):
        ...
        params["modules_target_value"] = jax.tree_util.tree_map(
            lambda x: x, params["modules_value"]
        )
