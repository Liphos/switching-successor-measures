...
class FBpiSwitchNonHierarchicalAgent(flax.struct.PyTreeNode):
    ...
    def successor_measure_extract(
        self, observations, z_goals, z_intents, module, grad_params=None
    ):
        # module is either "forward_repr" or "target_forward_repr"
        z_intents = self.normalize_z(z_intents)
        forward_reps = self.network.select(module)(
            observations, z_intents, goal_encoded=True, params=grad_params
        )
    ...
    def value_loss(self, batch, rng, grad_params):
        ...
        z_dummy_goals = self.network.select("backward_repr")(
            dummy_goals, params=grad_params
        )
        _, z = self.sample_latents(
            batch["intention_goals"], rng, self.config["critic_latent_mix_prob"]
        )  # TOreplace z and _
        ...
        # TD-error
        (next_v1, next_v2) = self.successor_measure_extract(
            next_obs, z_dummy_goals, z, "target_forward_repr"
        )
        ...
        (v1, v2) = self.successor_measure_extract(
            obs, z_dummy_goals, z, "forward_repr", grad_params=grad_params
        )
        v = (v1 + v2) / 2

        # Direction-error
        reward_z = (
            jnp.einsum("bd,bd->b", Bs_ng @ inv_cov, z) / self.config["latent_dim"]
        )[:, None]
        (next_v1_adv, next_v2_adv) = self.successor_measure_extract(
            next_obs, z, z, "forward_repr", grad_params=grad_params
        )
        next_v_adv = jnp.minimum(next_v1_adv, next_v2_adv)
        q_adv = reward_z + self.config["discount"] * next_v_adv

        (v1_adv, v2_adv) = self.successor_measure_extract(
            obs, z, z, "forward_repr", grad_params=grad_params
        )
        ...
        return value_loss, {
            ...
            "V_pred_mean": v.mean(),
            "V_target_mean": ((q1 + q2) / 2).mean(),
            "A_z_mean": adv.mean(),
            "A_z_pos_frac": (adv >= 0).mean(),
            "r_z_mean": reward_z.mean(),
            "v_max": v.max(),
            "v_min": v.min(),
            "B_norm": jnp.linalg.norm(Bs_ng, axis=-1).mean(),
            "z_norm": jnp.linalg.norm(z, axis=-1).mean(),
        }

    def orthonormal_loss(self, batch, grad_params):
        Bs = self.network.select("backward_repr")(
        ...
        diag_term = 2 * jnp.mean(norm_sq)
        off_diag_term = jnp.mean(dot_sq)
        loss = jnp.mean(dot_sq) - 2 * jnp.mean(norm_sq)

        return loss, {
            "ortho_loss": loss,
            "ortho_off_diag_sq": off_diag_term,
            "ortho_diag_term": diag_term,
        }

    def actor_loss(self, batch, rng, grad_params):
        """Compute the low-level actor loss."""
        z_norm, z = self.sample_latents(
            batch["actor_goals"], rng, self.config["actor_latent_mix_prob"]
        )

        v1, v2 = self.successor_measure_extract(
            batch["observations"], z, z, "forward_repr"
        )
        nv1, nv2 = self.successor_measure_extract(
            batch["next_observations"], z, z, "forward_repr"
        )
        ...
        return actor_loss, {
            ...
            "actor_adv_std": adv.std(),
            "actor_weight_mean": exp_a.mean(),
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        loss = (
            value_loss + self.config["orthonorm_coeff"] * orthonormal_loss + actor_loss
        )
    ...
    def sample_latents(self, targets, rng, latent_mix_prob):
        ...
        latents = jax.random.normal(
            latent_rng, shape=(batch_size, self.config["latent_dim"])
        )
    ...
    def create(cls, seed, ex_batch, config):
        ...
        network_info = dict(
            forward_repr=(
                forward_repr_def,
                (ex_observations, ex_latents, None, None, True),
            ),
            backward_repr=(backward_repr_def, (ex_observations,)),
            target_forward_repr=(
                copy.deepcopy(forward_repr_def),
                (ex_observations, ex_latents, None, None, True),
            ),
        ...
        network_params["modules_target_forward_repr"] = copy.deepcopy(
            network_params["modules_forward_repr"]
        )
        network_params["modules_target_backward_repr"] = copy.deepcopy(
            network_params["modules_backward_repr"]
        )
    ...
    config = ml_collections.ConfigDict(
        dict(
            ...
            forward_repr_hidden_dims=(
                ...
                512,
                512,
            ),  # Forward representation network hidden dimensions.
