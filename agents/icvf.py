...
class ICVFAgent(flax.struct.PyTreeNode):
    ...
    def repr_loss(self, batch, grad_params):
        ...
        intention_q = (
            intention_rewards
            + self.config["discount"] * intention_masks * next_intention_v
        )
    ...
    def infer_latent(self, batch):
        ...
        phis = self.network.select("repr")(
            batch["observations"], goals=None, intentions=None, info=True
        )[1]
        phis = phis[0]
    ...
    def create(
        ...
        network_info = dict(
            ...
            target_repr=(
                copy.deepcopy(repr_def),
                (ex_observations, ex_goals, ex_goals),
            ),
            target_critic=(
