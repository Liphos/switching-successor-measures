...
class FBpiSwitchAgent(flax.struct.PyTreeNode):
    ...
    def load_agent_from_frozen(self, FLAGS, config, example_batch):
        ...
        FBpiSwitchNonHierarchical = FBpiSwitchNonHierarchicalAgent.create(
            FLAGS.seed, example_batch, frozen_config
        )
        FBpiSwitchNonHierarchical = restore_agent(
            FBpiSwitchNonHierarchical, config["frozen_path"], config["restore_epoch"]
        )
    ...
    def successor_measure_extract(
        self, observations, z_goals, z_intents, module="forward_repr"
    ):
    ...
    def high_actor_loss(self, batch, grad_params, rng):
        ...
        latents_norm, latents = jax.lax.stop_gradient(
            self.sample_latents(subgoals, rng, 0.0)
        )
        z_goals_norm, z_goals = jax.lax.stop_gradient(
            self.sample_latents(goals, new_rng, self.config["critic_latent_mix_prob"])
        )
