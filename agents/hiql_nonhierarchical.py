...
class HIQLNonHierarchicalAgent(flax.struct.PyTreeNode):
    """Hierarchical implicit Q-learning (HIQL) without hierarchy that we call IQL agent (not really IQL)."""
    ...
    def actor_loss(self, batch, grad_params):
        ...
        goal_reps = self.network.select("goal_rep")(
            jnp.concatenate([batch["observations"], batch["actor_goals"]], axis=-1),
            params=grad_params,
        )
    ...
    def create(cls, seed, ex_batch, config):
        ...
        target_value_encoder = GCEncoder(
