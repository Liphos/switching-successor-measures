...
class FBHierarchicalAgent(flax.struct.PyTreeNode):
    ...
    def load_agent_from_frozen(self, FLAGS, config, example_batch):
        ...
        FB_agent = restore_agent(
            FB_agent, config["frozen_path"], config["restore_epoch"]
        )
    ...
    def successor_measure_extract(self, observations, z_goals, z_intents):
        ...
        forward_reps = self.network.select("margin_forward_repr")(
            observations, z_intents, goal_encoded=True
        )
    ...
    def marginalize_forward_loss(self, batch, grad_params, rng):
        ...
        latents_norm, _ = self.sample_latents(
            batch["observations"],
            rng,
            self.config["margin_latent_mix_prob"],
            permute_bool=True,
        )
        ...
        dist = self.network.select("actor")(
            observations, latents_norm, goal_encoded=True
        )
        ...
        target_forward_repr = jax.vmap(
            lambda a: self.network.select("forward_repr")(
                observations, latents_norm, actions=a, goal_encoded=True
            )
        )(actions)
        ...
        margin_forward_repr = self.network.select("margin_forward_repr")(
            observations, latents_norm, goal_encoded=True, params=grad_params
        )
        marginalization_loss = jnp.mean(
            (target_forward_repr - margin_forward_repr) ** 2
        )
    ...
    def high_actor_loss(self, batch, grad_params, rng):
        ...
        latents_norm, latents = jax.lax.stop_gradient(
            self.sample_latents(subgoals, rng, 0.0, permute_bool=False)
        )
        z_goals_norm, z_goals = jax.lax.stop_gradient(
            self.sample_latents(
                goals,
                new_rng,
                self.config["critic_latent_mix_prob"],
                permute_bool=False,
            )
        )
    ...
    def total_loss(self, batch, grad_params, rng=None):
        ...
        margin_loss, margin_info = self.marginalize_forward_loss(
            batch, grad_params, marginalize_rng
        )
    ...
    def sample_latents(self, targets, rng, latent_mix_prob, permute_bool=False):
        ...
        latent_backward_reprs = jax.lax.cond(
            ...
            lambda _: backward_reprs[
                jax.random.permutation(perm_rng, jnp.arange(batch_size))
            ],
            lambda _: backward_reprs,
            operand=None,
        )
        ...
        norm_latents = jnp.where(
            flags_latents, norm_latents, norm_latent_backward_reprs
        )
